...
const StatusBox = () => {
  ...
  useEffect(() => {
    ...
    const fetchCellData = async () => {
      const res = await fetch('https://mainnet-api.explorer.nervos.org/api/v1/daily_statistics/live_cells_count-dead_cells_count',{
        headers: {
          'Accept': 'application/vnd.api+json',
          'Content-Type': 'application/vnd.api+json'
      },
     ...
     setLiveCell((cellData[cellData.length-1].attributes.live_cells_count/1000000).toFixed(2))
    };
    const fetchAddressData = async () => {
      const res = await fetch('https://mainnet-api.explorer.nervos.org/api/v1/daily_statistics/addresses_count',{
        headers: {
          'Accept': 'application/vnd.api+json',
          'Content-Type': 'application/vnd.api+json'
      },
      ...
      setAddressCount((addressData[addressData.length-1].attributes.addresses_count/1000000).toFixed(2))
    };
    fetchData()
    fetchCellData()
    fetchAddressData()
